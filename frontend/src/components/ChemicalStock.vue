  ...
  <div class="cs-wrap">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        <h2>Chemical Stock Inventory</h2>
      </div>
      <div class="toolbar-right">
        <label class="toggle-filter" :class="{ active: showLowOnly }" @click="toggleFilter('low')">⚠ Low Stock</label>
        <label class="toggle-filter" :class="{ active: showExpiring }" @click="toggleFilter('expiring')">⏰ Expiring</label>
        <button class="btn btn-primary btn-sm" @click="showCreate = true">+ New Chemical</button>
    ...
    <div v-else-if="rows.length === 0" class="empty-hint">
      No chemicals yet. Click <strong>+ New Chemical</strong> to register one (Polyol, Isocyanate, etc.).
    </div>
    ...
    <div v-else class="inventory-grid">
      <div
        ...
        :key="row.id"
        class="inv-card"
        :class="{ 'low-stock': isLow(row), 'active-card': activeId === row.id, 'expired': isExpired(row) }"
        @click="openCard(row)"
      >
        <div class="inv-card-header">
          ...
          <div class="badge-stack">
            ...
            <span v-if="isExpired(row)" class="exp-badge expired">Expired</span>
            <span v-else-if="isExpiring(row)" class="exp-badge expiring">Expiring</span>
          </div>
        </div>
        ...
        <!-- Meta -->
        <div class="inv-meta">
          <span v-if="row.batch_number">Batch: {{ row.batch_number }}</span>
          <span v-if="row.expiry_date" :class="{ 'exp-text': isExpiring(row) || isExpired(row) }">
            Exp: {{ fmtDate(row.expiry_date) }}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="action-drawer" v-if="activeRow">
      ...
      <div v-if="drawerTab === 'add'" class="drawer-form">
        <div class="form-row">
          <div class="form-group">
            <label>Quantity to Add ({{ activeRow.unit }}) *</label>
            <input v-model.number="addForm.quantity" type="number" min="0.01" step="0.01" placeholder="e.g. 1000" />
          </div>
          <div class="form-group">
            <label>Batch Number</label>
            <input v-model="addForm.batch_number" placeholder="Supplier batch / lot no." />
          </div>
          <div class="form-group">
            <label>Expiry Date</label>
            <input v-model="addForm.expiry_date" type="date" />
          </div>
        </div>
        <div class="form-row">
          <div class="form-group flex-1">
            <label>Notes / Reference</label>
            <input v-model="addForm.notes" placeholder="Supplier, PO/challan no." />
          </div>
      ...
      <div v-if="drawerTab === 'remove'" class="drawer-form">
        <div class="form-row">
          <div class="form-group">
            <label>Quantity to Remove ({{ activeRow.unit }}) *</label>
            <input v-model.number="removeForm.quantity" type="number" min="0.01" step="0.01" />
          </div>
          <div class="form-group flex-1">
            <label>Reason / Notes</label>
            <input v-model="removeForm.notes" placeholder="Batch consumed, wastage…" />
          </div>
      ...
      <div v-if="drawerTab === 'adjust'" class="drawer-form">
        <div class="form-row">
          <div class="form-group">
            <label>New Actual Quantity ({{ activeRow.unit }}) *</label>
            <input v-model.number="adjustForm.new_quantity" type="number" min="0" step="0.01" :placeholder="activeRow.quantity_in_stock" />
          </div>
          <div class="form-group flex-1">
            <label>Reason *</label>
            <input v-model="adjustForm.reason" placeholder="Physical count, spillage…" />
          </div>
        ...
        <div class="avail-hint" v-if="adjustForm.new_quantity !== null">
          ...
          <strong :class="{ positive: adjustDiff > 0, negative: adjustDiff < 0 }">
            {{ adjustDiff > 0 ? '+' : '' }}{{ fmtQty(adjustDiff) }} {{ activeRow.unit }}
          </strong>
        </div>
      </div>
      ...
      <div v-if="drawerTab === 'history'" class="drawer-form">
        ...
        <table v-else class="history-table">
          <thead>
            <tr><th>Date</th><th>Type</th><th class="text-right">Qty</th><th>Notes</th></tr>
          </thead>
          <tbody>
    ...
    <div v-if="showCreate" class="modal-overlay" @click.self="showCreate = false">
      <div class="modal-box">
        ...
        <div class="form-group">
          ...
          <input v-model="createForm.name" placeholder="e.g. Polyol, Isocyanate (MDI)" />
        </div>
        <div class="form-row">
          <div class="form-group flex-1">
          ...
          <div class="form-group">
            ...
            <input v-model.number="createForm.reorder_level" type="number" min="0" step="0.01" />
          </div>
        </div>
        ...
        <div class="modal-actions">
          ...
          <button class="btn btn-primary" :disabled="creating || !createForm.name || !createForm.unit" @click="doCreate">
            {{ creating ? 'Creating…' : 'Create Chemical' }}
          </button>
        </div>
      </div>
    </div>
...
const addForm     = reactive({ quantity: null, batch_number: '', expiry_date: '', notes: '' })
...
function toggleFilter(which) {
  if (which === 'low')      showLowOnly.value  = !showLowOnly.value
  if (which === 'expiring') showExpiring.value = !showExpiring.value
  load()
}
...
function openCard(row) {
  ...
  activeRow.value = row
  drawerTab.value = 'add'
  Object.assign(addForm,    { quantity: null, batch_number: '', expiry_date: '', notes: '' })
...
async function loadHistory() {
  ...
  try {
    const res  = await stockService.getChemical(activeRow.value.id)
...
async function doAdd() {
  ...
  try {
    const res = await stockService.addChemical(activeRow.value.id, {
    ...
    actionSuccess.value = `Added ${fmtQty(addForm.quantity)} ${activeRow.value.unit} to ${activeRow.value.name}.`
    Object.assign(addForm, { quantity: null, batch_number: '', expiry_date: '', notes: '' })
  } catch (e) {
...
async function doRemove() {
  submitting.value = true; actionError.value = null; actionSuccess.value = null
  try {
    const res = await stockService.removeChemical(activeRow.value.id, {
...
async function doAdjust() {
  submitting.value = true; actionError.value = null; actionSuccess.value = null
  try {
    const res = await stockService.adjustChemical(activeRow.value.id, {
...
async function doCreate() {
  ...
  try {
    await stockService.createChemical({
      name:          createForm.name,
      category:      createForm.category || null,
      unit:          createForm.unit,
      reorder_level: createForm.reorder_level || 0,
    })
    showCreate.value = false
    Object.assign(createForm, { name: '', category: '', unit: 'kg', reorder_level: 0 })
    actionSuccess.value = 'Chemical registered.'
    await load()
  } catch (e) {
    createError.value = e?.response?.data?.message ?? 'Failed to create chemical.'
...
function isExpired(row)  { return row.expiry_date && new Date(row.expiry_date) < new Date() }
...
.inv-card.expired   { border-color: #ef9a9a; background: #fff7f7; }
...
.inv-category { font-size: 11px; color: #888; margin-top: 1px; }
...
.exp-badge    { font-size: 10px; font-weight: 700; border-radius: 8px; padding: 1px 7px; }
.exp-badge.expiring { background: #fff8e1; color: #f57f17; border: 1px solid #ffe082; }
.exp-badge.expired  { background: #ffebee; color: #c62828; border: 1px solid #ef9a9a; }
...
.exp-text { color: #e65100; font-weight: 600; }
