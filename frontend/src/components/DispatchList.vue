  ...
  <div class="dl-wrap">
    ...
    <div class="table-wrap">
      ...
      <table v-else>
        ...
        <tbody>
          <skeleton-rows v-if="loading" :rows="8" :cols="8" />
          <tr v-else-if="rows.length === 0"><td colspan="8" class="empty-row">No dispatches found.</td></tr>
          <tr v-for="row in rows" v-else :key="row.id" class="clickable" tabindex="0" role="button" :aria-label="'Open ' + row.dispatch_no" @click="$emit('view', row.id)" @keyup.enter="$emit('view', row.id)">
