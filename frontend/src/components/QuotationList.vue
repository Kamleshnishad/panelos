    ...
    <div class="table-wrap">
      ...
      <table v-else>
        ...
        <tbody>
          ...
          <tr v-for="row in rows" v-else :key="row.id" class="clickable" tabindex="0" role="button" :aria-label="'Open ' + row.quotation_no" @click="$emit('view', row.id)" @keyup.enter="$emit('view', row.id)">
