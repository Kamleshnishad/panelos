  ...
  <div class="qd-wrap" v-if="quotation">
    ...
    <div class="qd-toolbar">
      ...
      <div class="action-bar">
        ...
        <button class="btn btn-ghost" :disabled="pdfLoading" @click="downloadPdf" title="Download full quotation PDF">⬇</button>
        <button class="btn btn-boq" :disabled="boqLoading" @click="openBoq" title="BOQ cutting sheet for workers (no rates)">{{ boqLoading ? 'Opening…' : '🔧 BOQ Sheet' }}</button>
        <button v-if="quotation.status === 'draft'" class="btn btn-edit" @click="$emit('edit', quotation.id)">Edit</button>
        <button v-if="quotation.status === 'draft'" class="btn btn-send" :disabled="actionLoading || quotation.rates_pending" :title="quotation.rates_pending ? 'Enter rates before sending' : 'Send to customer'" @click="action('send')">Send</button>
        <button v-if="quotation.status === 'sent'"  class="btn btn-accept" :disabled="actionLoading" @click="action('accept')">Accept</button>
        <button v-if="['draft','sent'].includes(quotation.status)" class="btn btn-reject" :disabled="actionLoading" @click="action('reject')">Reject</button>
        <button v-if="['sent','accepted'].includes(quotation.status)" class="btn btn-revise" :disabled="actionLoading" @click="action('revise')">Revise</button>
        <button v-if="quotation.status === 'accepted'" class="btn btn-order" :disabled="actionLoading" @click="action('create-order')">Create Order</button>
        <button class="btn btn-dup" :disabled="actionLoading" @click="action('duplicate')" title="Copy as new draft">Duplicate</button>
        <button v-if="['draft','sent'].includes(quotation.status)" class="btn btn-expire" :disabled="actionLoading" @click="action('expire')">Expire</button>
...
async function openBoq() {
  ...
  catch { actionError.value = 'Failed to open BOQ sheet.' }
...
async function action(type) {
  ...
  try {
    if (type === 'send')              { await quotationService.send(quotation.value.id);      actionSuccess.value = 'Quotation sent.' }
    else if (type === 'accept')       { await quotationService.accept(quotation.value.id);    actionSuccess.value = 'Quotation accepted.' }
    else if (type === 'reject')       { await quotationService.reject(quotation.value.id);    actionSuccess.value = 'Quotation rejected.' }
    else if (type === 'expire')       { await quotationService.expire(quotation.value.id);    actionSuccess.value = 'Quotation marked expired.' }
...
.btn-boq     { background: #eaf6f6; color: #0E8C8C; }
...
.btn-revise  { background: #f3e5f5; color: #6a1b9a; }
.btn-order   { background: var(--primary-tint); color: var(--primary); }
.btn-dup     { background: #e0f7fa; color: #006064; }
.btn-expire  { background: #fafafa; color: #777; border: 1px solid #ddd; }
