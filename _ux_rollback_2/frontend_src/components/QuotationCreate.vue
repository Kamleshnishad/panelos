      ...
      <section class="card">
        ...
        <div v-for="(row, ri) in panelRows" :key="row._key" class="panel-row-card">
          <div class="panel-row-header">
            ...
            <span class="row-summary">{{ rowSummary(row) }}</span>
          ...
          <div class="form-grid compact">
            ...
            <div class="form-group">
              ...
              <input v-model.number="row.density_kgm3" type="number" step="1" min="30" max="100" />
            </div>
          </div>
          ...
          <div class="skin-spec">
              ...
              <div class="form-grid compact">
                <div class="form-group">
                  ...
                  <select v-model="row.top_skin_material">
                    ...
                    <option value="GI">GI (Plain)</option>
            ...
            <div class="skin-col">
              ...
              <div class="form-grid compact">
                ...
                <div class="form-group">
                  <label>Surface</label>
                  <select v-model="row.bottom_surface" disabled>
                    <option value="PLAIN">Plain (always)</option>
                  </select>
                </div>
              </div>
            </div>
...
function onPanelTypeChange(row) {
  ...
  if (!pt) return
  // Auto-set surface based on panel type name
  const name = (pt.name || '').toLowerCase()
  if (name.includes('roof') || name.includes('wall-ribbed')) {
    row.top_surface = 'RIBBED'
  } else {
    row.top_surface = 'PLAIN'
  }
...
function rowSummary(row) {
  const pt = panelTypes.value.find(p => p.id === row.panel_type_id)
  if (!pt) return ''
  return `${pt.name} | ${row.thickness}mm ${row.density_type} | ${row.top_surface} | ₹ ${fmtNum(rowTotalAmount(row))}`
}
...
async function save(mode) {
  ...
  if (!form.customer_id) { submitError.value = 'Please select a customer.'; return }
  if (panelRows.value.length === 0) { submitError.value = 'Add at least one panel row.'; return }
  for (const row of panelRows.value) {
    if (!row.panel_type_id) { submitError.value = 'Select a panel type for each row.'; return }
    if (row.sizes.length === 0) { submitError.value = 'Each panel row needs at least one size entry.'; return }
    for (const sz of row.sizes) {
      if (!sz.length_mm || sz.length_mm < 500) { submitError.value = 'Length must be at least 500mm.'; return }
    }
  }
  ...
  try {
    const payload = {
      ...
      panel_rows: panelRows.value.map(row => ({
        ...
        top_surface: row.top_surface,
        bottom_skin_material: row.bottom_skin_material,
        bottom_skin_thickness: row.bottom_skin_thickness,
...
.panel-row-header { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
...
.row-summary { flex: 1; font-size: 12px; color: #555; font-style: italic; }
...
.skin-label { font-size: 10px; font-weight: 700; letter-spacing: 1px; margin-bottom: 8px; }
