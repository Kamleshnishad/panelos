...
<div style="margin-bottom:16px">
  <div class="boq-spec-bar">
    ...
    &nbsp; {{ $item->panelType->name ?? '—' }}
    @if($item->application) &nbsp; | <strong>{{ strtoupper($item->application) }}</strong> @endif
    &nbsp; | {{ $item->thickness }}mm {{ $item->density_type }} {{ $item->density_kgm3 }}kg/m³
    @if($item->fixing_system) &nbsp; | FIXING: {{ $item->fixing_system }} @endif
    &nbsp; | TOP: {{ $item->top_skin_thickness }}mm {{ $item->top_skin_material }} {{ $item->top_color }}@if($item->top_color_ral) RAL {{ $item->top_color_ral }}@endif ({{ $item->top_surface }})
    &nbsp; | BTM: {{ $item->bottom_skin_thickness }}mm {{ $item->bottom_skin_material }} {{ $item->bottom_color }}@if($item->bottom_color_ral) RAL {{ $item->bottom_color_ral }}@endif ({{ $item->bottom_surface ?? 'PLAIN' }})
