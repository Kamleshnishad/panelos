    ...
    <tbody>
      @foreach($items as $i => $item)
        ...
        <tr class="item-row">
          <td>{{ $i + 1 }}</td>
          <td>
            <div class="desc-text">SUPPLY OF {{ strtoupper($company->name) }} MAKE
              @if(in_array($item->density_type, ['PUF','PIR']))CONTINUOUS LINE @endif{{ strtoupper($item->density_type) }} PANEL
              @if($item->application) — {{ strtoupper($item->application) }}@endif
              {{ $item->thickness }}MM THICK.</div>
            @if($pt && $pt->name)<div class="desc-detail" style="font-weight:bold;color:#2B50E0">{{ strtoupper($pt->name) }}</div>@endif
            <div class="desc-detail">
              Core: {{ $item->density_type }} {{ $item->density_kgm3 }}(±2) kg/m³
              @if($item->fixing_system) &nbsp;&bull;&nbsp; Fixing: {{ $item->fixing_system }} @endif
              &nbsp;&bull;&nbsp; Top: {{ $item->top_skin_thickness }}mm {{ $item->top_skin_material }} {{ strtoupper($item->top_color) }}@if($item->top_color_ral) RAL {{ $item->top_color_ral }}@endif ({{ $item->top_surface }})
              &nbsp;&bull;&nbsp; Bottom: {{ $item->bottom_skin_thickness }}mm {{ $item->bottom_skin_material }} {{ strtoupper($item->bottom_color) }}@if($item->bottom_color_ral) RAL {{ $item->bottom_color_ral }}@endif ({{ $item->bottom_surface ?? 'PLAIN' }})
